...
Foods & assumptions now contains editable food/portion and cooking-detail fields. Re-estimate with corrections explicitly sends the same photo with the corrected description (up to 6,000 characters including section labels), updates the editable totals on success, and caches the revision. Ordinary navigation still uses the cache. Failed revisions retain the previous usable result, including across restart. This is available while reviewing an unconfirmed meal; saving still requires confirmation. Corrections may improve an estimate but do not establish ground-truth accuracy.
